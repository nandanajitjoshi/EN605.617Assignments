...
int main(){
    ...
    checkCudaErrors (cudaMemcpy(val_dvc, val_host, rows*sizeof(double), cudaMemcpyHostToDevice ));
    ...
    cusparseCreateBsrilu02Info(&info_coeff);



    //checkCudaErrors (cusparseStatus);

    checkCudaErrors(cusparseDbsrilu02_bufferSize(handle, dir_coeff,  rows, rows,
    ...
    checkCudaErrors(cusparseDbsrilu02_analysis(handle, dir_coeff, rows, rows,
    ...
    checkCudaErrors(cusparseDbsrilu02(handle, dir_coeff, rows, rows,
               descr_coeff, val_dvc,I_dvc, J_dvc, dimBlock, info_coeff, policy_coeff, pBuffer));
